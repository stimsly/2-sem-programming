		...
		if (b >= 'a' and b <= 'z') {
			...
			if (len) {
				...
				if (is[a[len - 1] - 'a']) {
					...
					if(!vowels.was(a, len))vowels.push_back(a, len, v, fout);
					//cout << endl;
				}
				else {
					fout << "ѕроихводитс€ вставка слова с список где на конце согласна€\n";
					fout << " оличество согласных: " << len - v << endl;
					if(!consonants.was(a, len))consonants.push_back(a, len, len - v, fout);
					//cout << endl;
				}
				//cout << endl;
				len = 0;
